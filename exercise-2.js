...
const obj = {
  ...
  alert(obj.name);
  // đối tượng anotherObj:
  let clone ={anotherObj};
  for (let key in anotherObj) {
      clone[key] =anotherObj[key];
  }
  clone.name = 'Trainee';

  alert(anotherObj.name);
...
const clone = newObj . assign(Object,anotherObj);
  ...
  */
  ...
  let numbersMap = new Map([
      ["whole numbers":[1,2,3,4] ]
      ["Decimals numbers":[1.1,1.2,1.3,1.4]]
      ["negative numbers":[-1,-2,-3,-4]]
  ]);
  ...
  Map.set([1,2,3,4]);
  Map.set([1.1,1.2,1.3,1.4]);

  alert(Map.get(1,2,3,4));
  alert(Map.get(1.1,1.2,1.3,1.4));
